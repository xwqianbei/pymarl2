...
class LLMAgent(nn.Module):
    def __init__(self, input_shape, args):
        ...
        self.role_selector = nn.sequential(
            nn.Linear(args.rnn_hidden, args.role_selector_dim),
            nn.ReLU(),
    ...
    def forward(self, inputs, hidden_state):
        ...
        role_embedding = self.role_embeddings[role_idx]

        policy_w = self.role_decoder_w(role_embedding).reshape(-1, self.args.rnn_hidden_dim, self.args.n_actions) # [bs * a, rnn_hidden_dim * n_actions]
        policy_b = self.role_decoder_b(role_embedding) # [bs * a, n_actions]
